...
class RetrievalEvaluator:
    ...
    def evaluate(self, query: str, chunks: List[DocumentChunk]) -> EvaluationResult:
        if self.client is None:
            return EvaluationResult(
                ...
                confidence=0.35,
                reasons=["Gemini client unavailable or API key missing."],
                cleaned_context=[c.text for c in chunks[:3]],
        ...
        response = self.client.models.generate_content(
            model=self.model,
            contents=prompt,
            config={"temperature": self.temperature, "max_output_tokens": self.max_output_tokens},
        )
    ...
    def _build_prompt(self, query: str, chunks: List[DocumentChunk]) -> str:
        payload = {
            ...
            "query": query,
            "retrieved_chunks": [
                {"chunk_id": c.chunk_id, "source": c.source, "text": c.text, "metadata": c.metadata}
                for c in chunks
            ],
...
".join(parts).strip()
    ...
    def _rewrite_query_local(self, query: str) -> str:
        return f"{query.strip().rstrip('?')} official updated policy or authoritative guidance"
